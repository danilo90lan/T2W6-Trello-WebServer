...
class CardSchema(ma.Schema):
    ...
    # we set the allowed characters for the title
    title = fields.String(required=True, validate=And(Length(min=4, error="Title must be at least 4 characters in length."), 
                                                      Regexp("^[A-Z][A-Za-z0-9 ]+$", error="Title must start with a capital letter and have alphanumeric characters only.")))

    # we check if the entered value is contained in the constant tuple
